...
asmlinkage long interceptor(struct pt_regs reg) {
    // generic interceptor function is what each intercepted syscall will reach
    int intercepted_syscall = reg.ax;

    //If partial monitoring, check if pid is in whitelist
    if ((table[intercepted_syscall].monitored == 1) &&
        (check_pid_monitored(intercepted_syscall, current->pid) == 1)) {
        log_message(current->pid, reg.ax, reg.bx,reg.cx,
                    reg.dx, reg.si, reg.di, reg.bp);

        //If entirely monitoring, check if pid is not in blacklist
    } else if ((table[intercepted_syscall].monitored == 2) &&
               (check_pid_monitored(intercepted_syscall, current->pid) == 0)) {
        log_message(current->pid, reg.ax, reg.bx,reg.cx,
                    reg.dx, reg.si, reg.di, reg.bp);
    }

    return table[intercepted_syscall].f(reg);
}
    ...
    switch (cmd) {
        case REQUEST_SYSCALL_INTERCEPT :
            if (syscall < 0 || syscall > NR_syscalls ||
                syscall == MY_CUSTOM_SYSCALL || syscall == __NR_exit_group) { // Check valid syscall (-EINVAL)
                return -EINVAL;
            }

            if (current_uid() != PROCESS_ROOT) { // Check valid permissions (-EPERM)
                return -EPERM;
            }

            spin_lock(&calltable_lock);
            if (table[syscall].intercepted != 0) { // Check if has been intercepted (-EBUSY)
        ...
        case REQUEST_SYSCALL_RELEASE :
            ...
            spin_lock(&calltable_lock);
            if (table[syscall].intercepted != 1) { // Check if has been intercepted (-EINVAL)
                spin_unlock(&calltable_lock);
                return -EINVAL;
            }
        ...
        case REQUEST_START_MONITORING :
            if (syscall < 0 || syscall > NR_syscalls ||
                syscall == MY_CUSTOM_SYSCALL) { // Check valid syscall (-EINVAL)
                return -EINVAL;
            }

            if ((pid < 0) || ((pid != PID_ALL) &&
                              (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL))) { // Check valid pid (-EINVAL)
                return -EINVAL;
            }

            if (current_uid() != PROCESS_ROOT) { // Check valid permissions (-EPERM)
                if (pid == PID_ALL ||
                    check_pid_from_list((pid_t)pid, current->pid) != 0) {
                    return -EPERM;
                }
            }

            spin_lock(&calltable_lock);
            if (table[syscall].intercepted == 0) { // Check if has been intercepted (-EINVAL)
                spin_unlock(&calltable_lock);
                return -EINVAL;
            }
            spin_unlock(&calltable_lock);

            spin_lock(&pidlist_lock); // Following need lock both table & pidlist, choose to use pidlist_lock
            if (pid == PID_ALL) { // Monitor all pids
                if (table[syscall].monitored == 2 &&
                    table[syscall].listcount == 0) { // All pids has been monitored (-EBUSY)
                    spin_unlock(&pidlist_lock);
                    return -EBUSY;
                }

                if (table[syscall].listcount != 0) { // Empty my_list
                    destroy_list(syscall);
                }

                // Monitoring all pids and change to blacklist
            ...
            // When pid != 0
            if (table[syscall].monitored == 2) { // Blacklist case:
                if (check_pid_monitored(syscall, pid) == 0) { // Pid is not in blacklist: be monitored (-EBUSY)
                    spin_unlock(&pidlist_lock);
                    return -EBUSY;
                } else { // Pid is in the blacklist: not be monitored
                    if (del_pid_sysc(pid, syscall) != 0) { // Delete from blacklist
                        spin_unlock(&pidlist_lock);
                        return -EINVAL;
                    }
                }
            } else if (table[syscall].monitored == 1) { // When monitored == 1
                if (check_pid_monitored(syscall, pid) == 0) { // Pid is not in white list: not be monitored
                    if (add_pid_sysc(pid, syscall) != 0) { // Add pid into list and check (-ENOMEM)
                        spin_unlock(&pidlist_lock);
                        return -ENOMEM;
                    }
                } else { // Pid is in white list: be monitored (-EBUSY)
                    spin_unlock(&pidlist_lock);
                    return -EBUSY;
                }
            } else { // When monitored == 0
                if (add_pid_sysc(pid, syscall) != 0) { // Add pid into list and check (-ENOMEM)
                    spin_unlock(&pidlist_lock);
                    return -ENOMEM;
                }
        ...
        case REQUEST_STOP_MONITORING :
            ...
            spin_lock(&pidlist_lock);
            if (pid == PID_ALL && table[syscall].monitored != 0) { // Stop monitoring all pids
                destroy_list(syscall);
                INIT_LIST_HEAD(&(table[syscall].my_list));
                spin_unlock(&pidlist_lock);
                return 0;
            ...
            spin_lock(&pidlist_lock); // Following need lock both table & pidlist, choose to use pidlist_lock
            if (table[syscall].monitored == 0) { // Check monitored (-EINVAL)
                spin_unlock(&pidlist_lock);
                return -EINVAL;
            } else if (table[syscall].monitored == 1) { // When monitored == 1 and stop specific pid
                if (check_pid_monitored(syscall, pid) == 0) { // Stopped pid is not in the list (-EINVAL)
                    spin_unlock(&pidlist_lock);
                    return -EINVAL;
                } else { // Stopped pid is in the list, can be deleted
                    if (del_pid_sysc(pid, syscall) != 0) {
                        spin_unlock(&pidlist_lock);
                        return -EINVAL;
                    }
                }
            } else { // When monitored = 2: blacklist case, stop specific pid
                if (check_pid_monitored(syscall, pid) != 0) { // Pid is in blacklist, not be monitored
                    spin_unlock(&pidlist_lock);
                    return -EINVAL;
                } else { // Pid is not in blacklist, be monitored (-ENOMEM)
...
static int init_function(void) {
    ...
    spin_lock(&calltable_lock);
    orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL]; //read
    orig_exit_group = sys_call_table[__NR_exit_group]; // read

    set_addr_rw((unsigned long)sys_call_table);
    sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall; // write
    sys_call_table[__NR_exit_group] = my_exit_group; // write
    set_addr_ro((unsigned long)sys_call_table);
    spin_unlock(&calltable_lock);
...
static void exit_function(void) {
    ...
    for (i = 0; i < NR_syscalls + 1; i++) {
        if (table[i].intercepted == 1) {
            spin_lock(&calltable_lock); // return any intercepted syscall
            set_addr_rw((unsigned long)sys_call_table);
        ...
        if (table[i].monitored != 0) {
            spin_lock(&pidlist_lock); // if monitored any pid, free them
            destroy_list(i);
            spin_unlock(&pidlist_lock);
        }
    }
